...
async def login_post(request:Request ,response:Response,db:AsyncSession=Depends(get_db), username:str=Form(...), password:str=Form(...)):
    ...
    if bcrypt.checkpw(byte,hash):
        ...
        return template.TemplateResponse("login.html", {"request":request, "message": "login failed, try again "})